...
const formValidations = {
  numberDocument:  [(v: string) => v.trim().length >= 4, 'Ingresa tu número de documento'],
  typeDocument:    [(v: string) => v.length > 0, 'Selecciona un tipo de documento'],
  ...
  password:        [(v: string) => v.length >= 8, 'Mínimo 8 caracteres'],
...
export const RegisterForm = ({ onSuccess, loading, error, onRegister }: Props) => {
  ...
  const {
    numberDocument, typeDocument, name, lastName, email, password, confirmPassword,
    onInputChange, isFormValid,
    numberDocumentValid, typeDocumentValid, nameValid, lastNameValid,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const ok = await onRegister({ numberDocument, typeDocument, name, lastName, email, password });
    if (ok) {
      setDone(true);
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label htmlFor="reg-doc-type">Tipo de documento</Label>
          <select
            id="reg-doc-type"
            name="typeDocument"
            value={typeDocument}
          ...
          {submitted && typeDocumentValid && <p className="text-xs text-red-500">{typeDocumentValid}</p>}
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="reg-doc-num">Número de documento</Label>
          <Input
            id="reg-doc-num"
            name="numberDocument"
            ...
            className={submitted && numberDocumentValid ? 'border-red-400' : ''}
          />
          {submitted && numberDocumentValid && <p className="text-xs text-red-500">{numberDocumentValid}</p>}
        </div>
      </div>
