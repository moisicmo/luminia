...
export const useAuth = () => {
  ...
  const login = async (email: string, password: string, systemId: string): Promise<boolean> => {
    setLoading(true);
    setError(null);
    try {
      const { data } = await luminiApi.post('/auth/login', {
        typeAuth: 'EMAIL',
        identifier: email,
        password,
        systemId,
  ...
  const register = async (payload: {
    ...
    setError(null);
    try {
      await luminiApi.post('/auth/register', {
