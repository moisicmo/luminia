...
public class PdfEltrInvoiceSectorEducativo extends PdfInvoice<XmlSectorEducativoEltrInvoice, XmlSectorEducativoHeader> implements PdfProxy {

    public PdfEltrInvoiceSectorEducativo(XmlSectorEducativoEltrInvoice invoice, String paperType, Image logo) {
        super(invoice, paperType, logo);
        this.fileName = String.format("/reports/facturaSectorEducativo_%s.jrxml", paperType);
        this.productCode = invoice.getDetalle().get(0).getCodigoProducto();
    }

    public HashMap<String, Object> getDefaultParametersForHeader(XmlSectorEducativoHeader header)
        throws IOException, WriterException {
        HashMap<String, Object> parameters = getDefaultParameters(header);
        parameters.put(PdfInvoiceParameters.INVOICE_PERIODO_FACTURADO, header.getPeriodoFacturado());
        parameters.put(PdfInvoiceParameters.INVOICE_NOMBRE_ESTUDIANTE, header.getNombreEstudiante());
