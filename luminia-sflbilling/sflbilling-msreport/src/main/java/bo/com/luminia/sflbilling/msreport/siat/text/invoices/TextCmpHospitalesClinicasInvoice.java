...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextCompraVentaTemplate;
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextHospitalesClinicasTemplate;
...
public class TextCmpHospitalesClinicasInvoice extends TextInvoice<XmlHospitalClinicaCmpInvoice> {

    public TextCmpHospitalesClinicasInvoice(XmlHospitalClinicaCmpInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextCompraVentaTemplate.HEADER_A4_2;
            }else {
                this.header = TextCompraVentaTemplate.HEADER_A4;
            }
            this.details = TextHospitalesClinicasTemplate.DETAILS_A4;
            this.footer = TextHospitalesClinicasTemplate.FOOTER_A4;
        } else {
            this.header = TextHospitalesClinicasTemplate.HEADER_POS;
            this.details = TextHospitalesClinicasTemplate.DETAILS_POS;
            this.footer = TextHospitalesClinicasTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateDetailsA4() {
        ...
        for (XmlHospitalClinicaDetail item : invoice.getDetalle()) {
            ...
            row.put(TextInvoiceParameters.QUIROFANO, item.getNroQuirofanoSalaOperaciones() != null ? item.getNroQuirofanoSalaOperaciones().toString() : "");
            row.put(TextInvoiceParameters.ESPECIALIDAD, item.getEspecialidad());
            row.put(TextInvoiceParameters.MEDICO, item.getNombreApellidoMedico());
            row.put(TextInvoiceParameters.ESPECIALIDAD_MED, item.getEspecialidadMedico());
            row.put(TextInvoiceParameters.NIT, item.getNitDocumentoMedico() != null ? item.getNitDocumentoMedico().toString() : "-");
            row.put(TextInvoiceParameters.NRO_FACTURA, item.getNroFacturaMedico() != null ? item.getNroFacturaMedico().toString() : "-");
