...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextEntidadesFinancierasTemplate;
...
public class TextEltrEntidadesFinancierasInvoice extends TextInvoice<XmlEntidadesFinancierasEltrInvoice> {

    public TextEltrEntidadesFinancierasInvoice(XmlEntidadesFinancierasEltrInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextEntidadesFinancierasTemplate.HEADER_A4_2;
            }else {
                this.header = TextEntidadesFinancierasTemplate.HEADER_A4;
            }
            this.details = TextEntidadesFinancierasTemplate.DETAILS_A4;
            this.footer = TextEntidadesFinancierasTemplate.FOOTER_A4;
        } else {
            this.header = TextEntidadesFinancierasTemplate.HEADER_POS;
            this.details = TextEntidadesFinancierasTemplate.DETAILS_POS;
            this.footer = TextEntidadesFinancierasTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateDetailsA4() {
        ...
        for (XmlEntidadesFinancierasDetail item : invoice.getDetalle()) {
    ...
    protected String[] generateFooterA4() {
        ...
        parameters.put(TextInvoiceParameters.FTR_MONTO_ARREND, invoice.getCabecera().getMontoTotalArrendamientoFinanciero() != null
            ? invoice.getCabecera().getMontoTotalArrendamientoFinanciero().toString() : " 0.00");
    ...
    protected String[] generateFooterPOS() {
        ...
        parameters.put(TextInvoiceParameters.FTR_MONEDA_LITERAL,TextInvoice.BOLIVIANOS);
        parameters.put(TextInvoiceParameters.HDR_CODIGO_SECTOR, invoice.getCabecera().getCodigoDocumentoSector().toString());
        parameters.put(TextInvoiceParameters.FTR_ARRENDAMIEN,
            invoice.getCabecera().getMontoTotalArrendamientoFinanciero() != null ? invoice.getCabecera().getMontoTotalArrendamientoFinanciero().toString() : "0.00");
        parameters.put(TextInvoiceParameters.FTR_LEYENDA, invoice.getCabecera().getLeyenda());
        parameters.put(TextInvoiceParameters.FTR_MODALIDAD_LEYENDA, this.modalidadLeyenda);
        parameters.put(TextInvoiceParameters.FTR_MONTO_GIFT, invoice.getCabecera().getMontoGiftCard() != null ? invoice.getCabecera().getMontoGiftCard().toString() : "0.00");
        parameters.put(TextInvoiceParameters.FTR_MONTO_DESCUENTO, invoice.getCabecera().getDescuentoAdicional() != null ? invoice.getCabecera().getDescuentoAdicional().toString() : "0.00");
