...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextComercializacionHidrocarburosTemplate;
import bo.gob.impuestos.sfe.invoice.xml.XmlDateUtil;
import bo.gob.impuestos.sfe.invoice.xml.detail.XmlHidrocarburosDetail;
import bo.gob.impuestos.sfe.invoice.xml.eltr.XmlHidrocarburosEltrInvoice;

import java.util.HashMap;

public class TextEltrComercializacionHidrocarburosInvoice extends TextInvoice<XmlHidrocarburosEltrInvoice> {

    public TextEltrComercializacionHidrocarburosInvoice(XmlHidrocarburosEltrInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextComercializacionHidrocarburosTemplate.HEADER_A4_2;
            }else {
                this.header = TextComercializacionHidrocarburosTemplate.HEADER_A4;
            }
            this.details = TextComercializacionHidrocarburosTemplate.DETAILS_A4;
            this.footer = TextComercializacionHidrocarburosTemplate.FOOTER_A4;
        } else {
            this.header = TextComercializacionHidrocarburosTemplate.HEADER_POS;
            this.details = TextComercializacionHidrocarburosTemplate.DETAILS_POS;
            this.footer = TextComercializacionHidrocarburosTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateHeaderA4() {
        ...
        parameters.put(TextInvoiceParameters.HDR_PLACA_B_SISA_VIN, invoice.getCabecera().getPlacaVehiculo());
        parameters.put(TextInvoiceParameters.HDR_TIPO_ENVASE, invoice.getCabecera().getTipoEnvase());
    ...
    protected String[] generateDetailsA4() {
        ...
        for (XmlHidrocarburosDetail item : invoice.getDetalle()) {
    ...
    protected String[] generateFooterA4() {
        ...
        parameters.put(TextInvoiceParameters.FTR_MONTO_LEY, invoice.getCabecera().getMontoTotalSujetoIvaLey317() != null ?
            invoice.getCabecera().getMontoTotalSujetoIvaLey317().toString() : "0.00");
