...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextHotelesTemplate;
import bo.gob.impuestos.sfe.invoice.xml.XmlDateUtil;
import bo.gob.impuestos.sfe.invoice.xml.detail.XmlHotelesDetail;
import bo.gob.impuestos.sfe.invoice.xml.eltr.XmlHotelEltrInvoice;

import java.util.HashMap;

public class TextEltrHotelesInvoice extends TextInvoice<XmlHotelEltrInvoice> {

    public TextEltrHotelesInvoice(XmlHotelEltrInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextHotelesTemplate.HEADER_A4_2;
            }else {
                this.header = TextHotelesTemplate.HEADER_A4;
            }
            this.details = TextHotelesTemplate.DETAILS_A4;
            this.footer = TextHotelesTemplate.FOOTER_A4;
        } else {
            this.header = TextHotelesTemplate.HEADER_POS;
            this.details = TextHotelesTemplate.DETAILS_POS;
            this.footer = TextHotelesTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateDetailsA4() {
        ...
        for (XmlHotelesDetail item : invoice.getDetalle()) {
            ...
            row.put(TextInvoiceParameters.DESCRIPCION, item.getDescripcion() + " " + item.getDetalleHuespedes());
