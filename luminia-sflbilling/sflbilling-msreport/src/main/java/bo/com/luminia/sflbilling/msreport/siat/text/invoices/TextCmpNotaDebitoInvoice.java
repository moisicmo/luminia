...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextNotaDebitoTemplate;
import bo.gob.impuestos.sfe.invoice.xml.XmlDateUtil;
import bo.gob.impuestos.sfe.invoice.xml.cmp.XmlNotaCreditoDebitoCmpInvoice;
import bo.gob.impuestos.sfe.invoice.xml.detail.XmlNotaCreditoDebitoDetail;

import java.util.HashMap;

public class TextCmpNotaDebitoInvoice extends TextInvoice<XmlNotaCreditoDebitoCmpInvoice> {

    private String footer_1, footer_2;

    public TextCmpNotaDebitoInvoice(XmlNotaCreditoDebitoCmpInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextNotaDebitoTemplate.HEADER_A4_2;
            }else {
                this.header = TextNotaDebitoTemplate.HEADER_A4;
            }
            this.details = TextNotaDebitoTemplate.DETAILS_A4;
            this.footer_1 = TextNotaDebitoTemplate.FOOTER_A4_1;
            this.footer_2 = TextNotaDebitoTemplate.FOOTER_A4_2;
        } else {
            this.header = TextNotaDebitoTemplate.HEADER_POS;
            this.details = TextNotaDebitoTemplate.DETAILS_POS;
            this.footer_1 = TextNotaDebitoTemplate.FOOTER_POS_1;
            this.footer_2 = TextNotaDebitoTemplate.FOOTER_POS_2;
        }
    }
    ...
    protected String[] generateHeaderA4() {
        ...
        parameters.put(TextInvoiceParameters.HDR_NRO_FACTURA, invoice.getCabecera().getNumeroNotaCreditoDebito().toString());
    ...
    protected String[] generateDetailsA4(Integer transaccion) {
        if (data.isEmpty()) {
            for (XmlNotaCreditoDebitoDetail item : invoice.getDetalle()) {

                HashMap<String, String> row = new HashMap<>();
                row.put(TextInvoiceParameters.CODIGO_PRODUCTO, item.getCodigoProducto());
                row.put(TextInvoiceParameters.CODIGO_DETALLE_TRANSACCION, item.getCodigoDetalleTransaccion().toString());
    ...
    protected String generateDetailsA4Print(Integer transaccion) {
        ...
        for (HashMap<String, String> item : data) {
            if (item.get(TextInvoiceParameters.CODIGO_DETALLE_TRANSACCION).equals(transaccion.toString())) {
    ...
    protected String[] generateFooterA4_() {
        ...
        parameters.put(TextInvoiceParameters.FTR_TOTAL_LITERAL, integerToLiteral(invoice.getCabecera().getMontoTotalOriginal().intValue()));
        ...
        parameters.put(TextInvoiceParameters.FTR_TOTAL_DEVUELTO, invoice.getCabecera().getMontoTotalDevuelto() != null ?
            invoice.getCabecera().getMontoTotalDevuelto().toString() : "0.00");

        parameters.put(TextInvoiceParameters.FTR_EFEC_CRE_DEB, invoice.getCabecera().getMontoEfectivoCreditoDebito() != null ?
            invoice.getCabecera().getMontoEfectivoCreditoDebito().toString() : "0.00");
    ...
    protected String generateFooterA4_2() {
        ...
        footer = replaceNumberValues(TextInvoiceParameters.FTR_EFEC_CRE_DEB, parameters.get(TextInvoiceParameters.FTR_EFEC_CRE_DEB), footer, parameters.get(TextInvoiceParameters.FTR_EFEC_CRE_DEB).length());
    ...
    protected String generateDetailsPOSPrint(Integer transaction) {
        String response = "";
        for (HashMap<String, String> item : data) {
            if (item.get(TextInvoiceParameters.CODIGO_DETALLE_TRANSACCION).equals(transaction.toString())) {
