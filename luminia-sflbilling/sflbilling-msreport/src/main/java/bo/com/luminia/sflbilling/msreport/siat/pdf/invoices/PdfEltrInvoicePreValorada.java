...
public class PdfEltrInvoicePreValorada extends PdfInvoice<XmlPrevaloradaEltrInvoice, XmlPrevaloradaHeader> implements PdfProxy {

    public PdfEltrInvoicePreValorada(XmlPrevaloradaEltrInvoice invoice, String paperType, Image logo) {
        super(invoice, paperType, logo);
        this.fileName = String.format("/reports/facturaPreValorada_%s.jrxml", paperType);
        this.productCode = invoice.getDetalle().get(0).getCodigoProducto();
    }

    public HashMap<String, Object> getDefaultParametersForHeader(XmlPrevaloradaHeader header)
