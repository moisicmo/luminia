...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextPrevaloradaTemplate;
import bo.gob.impuestos.sfe.invoice.xml.XmlDateUtil;
import bo.gob.impuestos.sfe.invoice.xml.detail.XmlPrevaloradaDetail;
import bo.gob.impuestos.sfe.invoice.xml.eltr.XmlPrevaloradaEltrInvoice;

import java.util.HashMap;

public class TextEltrPrevaloradaInvoice extends TextInvoice<XmlPrevaloradaEltrInvoice> {

    public TextEltrPrevaloradaInvoice(XmlPrevaloradaEltrInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextPrevaloradaTemplate.HEADER_A4_2;
            }else {
                this.header = TextPrevaloradaTemplate.HEADER_A4;
            }
            this.details = TextPrevaloradaTemplate.DETAILS_A4;
            this.footer = TextPrevaloradaTemplate.FOOTER_A4;
        } else {
            this.header = TextPrevaloradaTemplate.HEADER_POS;
            this.details = TextPrevaloradaTemplate.DETAILS_POS;
            this.footer = TextPrevaloradaTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateDetailsA4() {
        for (XmlPrevaloradaDetail item : invoice.getDetalle()) {
    ...
    protected String[] generateHeaderPOS() {
        ...
        if (invoice.getCabecera().getOptional() != null && !invoice.getCabecera().getOptional().isEmpty()) {
            int i = 1;
            for (Object item : invoice.getCabecera().getOptional()) {
