...
import bo.com.luminia.sflbilling.msreport.siat.text.invoices.templates.TextSectorEducativoTemplate;
...
import bo.gob.impuestos.sfe.invoice.xml.eltr.XmlSectorEducativoEltrInvoice;

import java.util.HashMap;

public class TextEltrSectorEducativoInvoice extends TextInvoice<XmlSectorEducativoEltrInvoice> {

    public TextEltrSectorEducativoInvoice(XmlSectorEducativoEltrInvoice invoice, String paperType) {
        ...
        if (paperType == TextInvoice.PAPER_A4) {
            if (invoice.getCabecera().getDireccion().length() > MAX_DIRECCION_LENGTH) {
                this.header = TextSectorEducativoTemplate.HEADER_A4_2;
            }else {
                this.header = TextSectorEducativoTemplate.HEADER_A4;
            }
            this.details = TextSectorEducativoTemplate.DETAILS_A4;
            this.footer = TextSectorEducativoTemplate.FOOTER_A4;
        } else {
            this.header = TextSectorEducativoTemplate.HEADER_POS;
            this.details = TextSectorEducativoTemplate.DETAILS_POS;
            this.footer = TextSectorEducativoTemplate.FOOTER_POS;
        }
    }
    ...
    protected String[] generateHeaderA4() {
        ...
        parameters.put(TextInvoiceParameters.HDR_NOMBRE_ALUMNO, invoice.getCabecera().getNombreEstudiante());
    ...
    protected String[] generateDetailsA4() {
        for (XmlSectorEducativoDetail item : invoice.getDetalle()) {
