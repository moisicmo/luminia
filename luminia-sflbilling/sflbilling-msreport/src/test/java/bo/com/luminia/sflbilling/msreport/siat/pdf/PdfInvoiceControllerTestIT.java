...
@SpringBootTest
public class PdfInvoiceControllerTestIT {

    @Autowired
    private  PdfGeneratorService generatorService ;

    @Test
    public void testGeneratePdfA4(){

        byte[] pdfReport = new byte[0];
        try {
            pdfReport = generatorService.exportReportPdf("466282576963A4262596E6C01AAE4730C7A62586A502512694CA8DC74", PdfInvoice.PAPER_A4);
        } catch (PdfInvoiceCouldBeGeneratedException e) {
            e.printStackTrace();
        }

        assertThat(pdfReport).isNotNull();
    }

    @Test
    public void testGeneratePdfRoll(){

        byte[] pdfReport = new byte[0];
        try {
            pdfReport = generatorService.exportReportPdf("466282576963A4262596E6C01AAE4730C7A62586A502512694CA8DC74", PdfInvoice.PAPER_ROLL);
        } catch (PdfInvoiceCouldBeGeneratedException e) {
            e.printStackTrace();
        }
...
/*
    ...
    public void myTest(){
        ...
        assertThat(invoice.size()).isGreaterThan(0);
    }
    ...
    private void createCompany(){
        ...
        company.setUserSiat("Luminia2020");
        company.setPasswordSiat("Luminia2020bol");
    ...
    public void test(){
        ...
        XmlBaseInvoice baseInvoice = XmlInvoiceAdapter.convert(invoiceIssueReq, company.getModalitySiat());

        InvoiceIssueRes response = issueService.emit(invoiceIssueReq, baseInvoice);
