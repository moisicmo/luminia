package bo.com.luminia.sflbilling.msbatch.siat.invoice.xml;
...
import bo.com.luminia.sflbilling.msbatch.web.rest.errors.spec.XmlFailedConversionException;
    ...
    public static XmlBaseInvoice convert(InvoiceIssueReq request, ModalitySiatEnum modality, final InvoiceLegendRepository legendRepository)
        ...
        try {
            if (request.getDocumentSectorType() == XmlSectorType.INVOICE_COMPRA_VENTA) {
                ...
                if (modality.getKey() == ModalitySiatEnum.COMPUTERIZED_BILLING.getKey()) {
                    response = new XmlBoletoAereoCmpInvoice();
                    ((XmlBoletoAereoCmpInvoice) response).convert(request.getHeader(), null);
                    ((XmlBoletoAereoHeader) response.getCabecera()).setLeyenda(legend);
                } else {
                    throw new XmlFailedConversionException(String.format("Emisión de la factura solo para: Modalidad Computarizada"));
                }
