...
import bo.com.luminia.sflbilling.msbatch.repository.InvoiceWrapperRepository;
import bo.com.luminia.sflbilling.msbatch.repository.WrapperRepository;
import bo.com.luminia.sflbilling.msbatch.web.rest.certification.request.InvoiceIssueReq;
...
@SpringBootTest
public class TestEmisionMasivaIT {

    @Autowired
    private CompanyRepository companyRepository;
    @Autowired
    private InvoiceWrapperRepository invoiceWrapperRepository;
    @Autowired
    ...
    public void test() throws Exception {
        ...
        long scheduleSettingId = 1001L;
        String token = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJhZG1pbiIsImF1dGgiOiJST0xFX0FETUlOLFJPTEVfVVNFUiIsImV4cCI6MTYzNjY5NjUwOH0.x4LKFsou9OPi5v_Trs7D3R53a4P5Q2YuD7qUtFnGqu-Bx2iNVVaULapCn-jrrAlTWKu56bMeLY3Jnm_Mogkw_A";
        String companyHash = "9ce9f485-9b86-4e60-82b0-65ce7daceece";

        // Actualiza la empresa para envio masivo, solo la primera vez.
//        Optional<Company> companyFromDb = companyRepository.findById(companyId);
//        if (companyFromDb.isPresent()) {
//            Company company = companyFromDb.get();
//            company.setPackageSend(true);
//            company.setEventSend(false);
...
//        }
        ...
        for (Integer sectorDocumentType : sectorDocumentTypeList) {
            ...
            for (int number = 1; number <= numberTests; number++) {
                ...
                InvoiceIssueReq requestInvoice = this.createInvoice(companyId, branchOfficeSiat, pointSaleSiat, sectorDocumentType);
                for (int i = 1; i <= numberInvoices; i++) {
                    ...
                    HttpEntity<InvoiceIssueReq> request = new HttpEntity<>(requestInvoice, headers);
                    String result = restTemplate.postForObject("http://localhost:8082/api/invoice/issue", request, String.class);
                }

                // Envío masivo de facturas.
                InvoiceMassiveReq requestMassive = new InvoiceMassiveReq();
                requestMassive.setCompanyId(companyId);
                requestMassive.setScheduleSettingId(scheduleSettingId);
                mockMvc
                    .perform(post("/api/invoice/massive")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(TestUtil.convertObjectToJsonBytes(requestMassive)))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.code").value(200));

                //Thread.sleep(10000);
                //invoiceWrapperRepository.deleteAll();
                //wrapperRepository.deleteAll();

                // Validación de envío masivo de facturas.
                InvoiceMassiveValidateReq requestMassiveValidate = new InvoiceMassiveValidateReq();
                requestMassiveValidate.setCompanyId(companyId);
                mockMvc
                    .perform(post("/api/invoice/massive/validate")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(TestUtil.convertObjectToJsonBytes(requestMassiveValidate)))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.code").value(200));
            }
    ...
    private InvoiceIssueReq createInvoice(Long companyId, Integer branchOfficeSiat, Integer pointSaleSiat, Integer sectorDocumentType) {
