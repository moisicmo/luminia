...
@XmlType(propOrder = {"nitEmisor", "razonSocialEmisor", "municipio", "telefono", "numeroFactura", "cuf", "cufd",
    ...
    "numeroDocumento", "complemento", "codigoCliente", "codigoMetodoPago", "numeroTarjeta", "montoTotalArrendamientoFinanciero",
...
public class XmlEntidadesFinancierasHeader extends XmlBaseHeaderInvoice {

    private BigDecimal montoTotalArrendamientoFinanciero;

    @XmlElement(name = "montoTotalArrendamientoFinanciero", nillable = true)
    public BigDecimal getMontoTotalArrendamientoFinanciero() {
        if (montoTotalArrendamientoFinanciero != null)
            return montoTotalArrendamientoFinanciero.setScale(2, BigDecimal.ROUND_HALF_EVEN);
        return null;
    }
    ...
    public void convert(Map<String, Object> header) {
        ...
        if (header.containsKey(XmlTags.HEADER_MONTO_TOTAL_ARRENDAMIENTO_FINANCIERO) && header.get(XmlTags.HEADER_MONTO_TOTAL_ARRENDAMIENTO_FINANCIERO) != null) {
            this.setMontoTotalArrendamientoFinanciero(new BigDecimal(
                    (String) header.get(XmlTags.HEADER_MONTO_TOTAL_ARRENDAMIENTO_FINANCIERO)));
        }
    }
}
